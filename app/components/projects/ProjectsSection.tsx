...
export default function ProjectsSection() {
  ...
  return (
    <>
      ...
      {/* CHIP FILTER BADGES */}
      <div className="p-2.5 mb-12 rounded-2xl border border-slate-200/80 bg-white/80 backdrop-blur-md flex flex-wrap justify-center gap-2 shadow-xs">
        {filtersWithCounts.map(f => {
          ...
          return (
            <Badge
              ...
              className="py-1.5 px-3.5 text-xs font-black cursor-pointer transition-all active:scale-95 shadow-2xs"
            >
              {f.label} ({f.count})
            </Badge>
          );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-20">
        {filteredProjects.map(project => {
          ...
          return (
            <Card
              key={project.id}
              className="rounded-3xl border border-slate-200/80 bg-white overflow-hidden flex flex-col justify-between h-full transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-indigo-500/10 hover:border-indigo-300"
            >
              <div>
                {/* Header Gradient */}
