...
const PAGES_CONFIG = {
  ...
  "lamp-stack": {
    ...
    notes: "We design LAMP installations utilizing prepared statements, hashed tokens, secure cookies, and customized apache modules to prevent SQLi and XSS flaws.",
    faq: [{ q: "Why use LAMP stack in 2026?", a: "LAMP stack power over 70% of the web and remains highly cost-effective, simple, and fast for relational data workloads." }]
  },
  "mern-stack": {
...
// Content Generator function for nested child pages
function getNestedPageContent(hubKey, childKey, title, desc) {
  let content = "";
  if (hubKey === "dsa") {
    content = `
      <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-8 shadow-sm">
        ...
        <p className="text-gray-600 mb-6 leading-relaxed">
          ...
          Here is my learning journal and notes on <strong>${title}</strong>:
        </p>
    ...
    `;
  } else if (hubKey === "system-design") {
    content = `
      <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-8 shadow-sm">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">High & Low Level Architecture Analysis</h2>
        <p className="text-gray-600 mb-6 leading-relaxed">
          Detailed notes, trade-offs, and scaling guidelines for <strong>${title}</strong> systems.
        </p>
    ...
    `;
  } else if (hubKey === "devops") {
    content = `
      <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-8 shadow-sm">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Continuous Integration & Infrastructure Notes</h2>
        <p className="text-gray-600 mb-6 leading-relaxed">
          Deployment guides, build commands, and script templates for <strong>${title}</strong> operations.
        </p>
        ...
        <div className="bg-slate-900 border border-gray-800 rounded-xl p-5 mb-8 font-mono text-xs text-emerald-400">
          <span className="text-gray-500 block mb-2"># CLI Cheat Sheet commands for ${title}</span>
          $ npm run build <br />
    ...
    content = `
      <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-8 shadow-sm">
        ...
        <p className="text-gray-600 mb-8 leading-relaxed">
          Review the engineering steps, security checkpoints, and database choices deployed for <strong>${title}</strong>.
        </p>
...
function executeSetup() {
  console.log("Starting Generation of all dynamic portfolio pages...");

  // Generate Hub & Authority pages
  Object.entries(PAGES_CONFIG).forEach(([slug, info]) => {
    ...
    const relativePath = `https://ajitdev.com/${slug}`;
      ...
      <section className="py-16 md:py-24 bg-gray-50/50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-8 shadow-sm">
            ...
            <ul className="list-disc list-inside text-sm text-gray-600 space-y-2 mb-6">
              ${info.roadmap.map(r => `<li>${r}</li>`).join("\n              ")}
            </ul>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              ${(info.resources || ["Official Manuals", "GitHub Templates", "Cheat Sheets"]).map(r => `<span className="px-3 py-1 bg-gray-50 border border-gray-100 rounded-lg text-xs font-medium text-gray-500">${r}</span>`).join("\n              ")}
            </div>
          </div>
          ...
          <div className="bg-indigo-50 border border-indigo-100/80 rounded-2xl p-6 mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              ]).map(lnk => `<Link href="${lnk.url}" className="px-3 py-2 bg-white hover:bg-indigo-600 hover:text-white border border-indigo-100 rounded-xl text-xs font-semibold text-indigo-600 transition-colors shadow-sm">${lnk.name}</Link>`).join("\n              ")}
            </div>
          </div>
...
`;
  ...
  // Generate Nested Hub pages (DSA, System Design, DevOps, Case Studies)
  const nestedHubs = [
  ...
  nestedHubs.forEach((hub) => {
    hub.list.forEach((child) => {
      ...
      const relativePath = `https://ajitdev.com/${hub.key}/${child.key}`;
      const contentHtml = getNestedPageContent(hub.key, child.key, child.name, child.desc);
...
`;
      ...
      fs.writeFileSync(path.join(dirPath, "page.tsx"), pageContent, "utf8");
      console.log(`Generated nested page: /${hub.key}/${child.key}`);
    });
  });

  console.log("Successfully completed Generation of all routes!");
}

module.exports = { executeSetup };
